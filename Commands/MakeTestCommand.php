...
final class MakeTestCommand implements \Siro\Core\Commands\CommandInterface {
    ...
    private function fromTrace(string $traceId, array $ignoreFields): int
    {
        ...
        // Analyze trace for side-effect awareness
        $hasDbWrites = false;
        ...
        // Source trace comment + side-effect warnings
        $sourceComment = 'Source trace: ' . $traceId . ' (' . $method . ' ' . $apiPath . ' → ' . $status . ')';
        $warningLines = [];
        if ($hasDbWrites) {
            $warningLines[] = ' * WARNING: Trace contains DB write operations — this test may produce side effects.';
        }
        if (isset($data['outbound_http']) && is_array($data['outbound_http']) && $data['outbound_http'] !== []) {
            $warningLines[] = ' * NOTE: Trace captured outbound HTTP calls. Consider mocking external services.';
        }
        $sideEffectBlock = '';
        if ($warningLines !== []) {
            $sideEffectBlock = "\n *\n" . implode("\n", $warningLines) . "\n *";
...
// {$sourceComment}{$sideEffectBlock}
// Generated by: php siro make:test --from-trace={$traceId}
// Auth: *NEVER* embed real tokens. Use \$this->authenticate() for test auth.
