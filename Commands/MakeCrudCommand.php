...
final class MakeCrudCommand implements \Siro\Core\Commands\CommandInterface
{
    ...
    public function run(array $args): int
    {
        ...
        if (!$withoutRepository) {
            if (!$this->generateRepository($repoName, $model, $table)) { $ok = false; }
        }

        // 4. Service (skip in simple mode)
        if (!$withoutService) {
            if (!$this->generateService($serviceName, $model, $repoName, $withoutRepository)) { $ok = false; }
        }
...
final class {$model} extends Model
{
    ...
    protected array \$fillable = [
        ...
        'description', 'category', 'sku', 'price',
        'address', 'city', 'notes',
    ];
}
...
PHP);
    ...
    private function generateRoutes(string $resource, string $controllerClass): bool
    {
        ...
        if (str_contains($content, $marker)) {
            ...
            $pattern = '/\s*\/\/ Generated by: php siro make:crud ' . preg_quote($resource, '/') . '.*?(?=\n\s*\/\/ Generated|$)/s';
            $content = (string) preg_replace($pattern, "\n", $content);
        }

        $rbacMutation = $this->withRbac
            ? "->middleware([JsonMiddleware::class, 'auth:admin'])"
            : "->middleware([JsonMiddleware::class])";

        $routes = <<<PHP
    // Generated by: php siro make:crud {$resource}
    \$router->get('/{$resourceLower}', [\App\Controllers\\{$controllerClass}::class, 'index']);
    \$router->get('/{$resourceLower}/{id}', [\App\Controllers\\{$controllerClass}::class, 'show']);
    \$router->post('/{$resourceLower}', [\App\Controllers\\{$controllerClass}::class, 'store'])
        {$rbacMutation};
    \$router->put('/{$resourceLower}/{id}', [\App\Controllers\\{$controllerClass}::class, 'update'])
        {$rbacMutation};
    \$router->delete('/{$resourceLower}/{id}', [\App\Controllers\\{$controllerClass}::class, 'delete'])
        {$rbacMutation};
